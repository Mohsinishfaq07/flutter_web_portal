    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['history'] is List) {
          setState(() {
            assets = List.from(responseData['history']);
            isLoading = false;
          });
        } else {
          setState(() {
      ...
      decoration: BoxDecoration(borderRadius: BorderRadius.circular(10),        color: AppTheme.scaffoldBackgroundColor,
        ...
        child: SingleChildScrollView(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const CustomHeading(title: 'Assets History  Statistics',showBackButton: true),
                    ...
                    rows: assets.map((asset){
                      return DataRow(
                        cells: [
                          ...
                          DataCell(
                            Text(
                              asset['action_type']?.toString() ?? 'N/A',
                              style: TextStyle(color: Colors.white),
                            ),
                          ),
                          DataCell(
                              Text(
                                asset['assetID']?.toString() ?? 'N/A',
                                style: TextStyle(color: Colors.white),
                              ),
                              ),
                          DataCell(
                          ...
                          DataCell(
                            Text(
                              asset['user_id']?.toString() ?? 'N/A',
                              style: TextStyle(color: Colors.white),
                            ),
                          ),
                          DataCell(
                            Text(
                              asset['user_name']?.toString() ?? 'N/A',
                              style: TextStyle(color: Colors.white),
                            ),
                          ),
                          DataCell(
                            Text(asset["details"]?.toString() ?? 'N/A',
                              style: TextStyle(color: Colors.white),
                            ),
