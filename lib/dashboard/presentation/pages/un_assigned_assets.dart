...
class UnAssignedAssets extends StatefulWidget {
  const UnAssignedAssets({super.key});

  @override
  State<UnAssignedAssets> createState() => _UnAssignedAssetsState();
}

class _UnAssignedAssetsState extends State<UnAssignedAssets> {
  ...
  Future<void> getUnAssignedAssets() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print(responseData);
        if (responseData['unassigned_assets'] is List) {
          setState(() {
            assets = List.from(responseData['unassigned_assets']);
            isLoading = false;
          });
        } else {
          setState(() {
